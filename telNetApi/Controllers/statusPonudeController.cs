...
{
    public class statusPonudeController : baseApiController<statusPonude>
    {
        public statusPonudeController(baseInterface<telNet.Models.statusPonude> depo) : base(depo){ }

        public IHttpActionResult Get()
        {
            try
            {
                return Ok(tnDepo.Get().ToList().Select(x => Factory.Create(x, true,true)));
            }
            catch (Exception ex)
            {
        ...
        public HttpResponseMessage Get(int id)
        {
            try
            {
                ...
                if (entity != null)
                    return Request.CreateResponse(HttpStatusCode.OK, Factory.Create(entity, true, true));
                else
        ...
        public HttpResponseMessage Post(statusPonudeModel dm)
        {
            try
            {
        ...
        public HttpResponseMessage Delete(int id)
        {
            try
            {
                statusPonude entity = tnDepo.get(id);
                if (entity == null)
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "No data");
